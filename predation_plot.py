  ...
  if (len(poplist)>1):
      ...
      for j in range(l):
          for i in range(l):
              ...
              if (d<eps and abs(bufp[j].p-bufp[i].p)<0.1 and flag[j] and flag[i] and i != j):
                  if (d<min_dist):
                      min_dist=d
                      key_closest = i
                  cflag=True
...
for i in range(0, 2500501, 500):
    ...
    #view_poplist = pop_clust2(poplist, 0.1)
    view_poplist = poplist
    pred = info(view_poplist)
    pdynamic.append(pred)
    qdynamic.append(len(view_poplist))
    if i%100000==0:
        print(f"{i} iterations have passed")
        

x = np.arange(0, len(pdynamic)*500, 500)

fig = plt.figure()
#plt.gca().set(xlim=(0, max(x)), ylim=(0.0, max(pdynamic)))
#plt.margins(x=0, y=0)
plt.plot(x, pdynamic)
#plt.title('Изменение максимального коэффициента хищничества с течением времени')
plt.xlabel('время')
plt.ylabel('максимальный коэффициент хищничества')

plt.savefig(outpass, dpi=300, bbox_inches='tight')

fig = plt.figure()
plt.gca().set(xlim=(0, max(x)), ylim=(0.0, max(qdynamic)))
plt.margins(x=0,y=0)
plt.plot(x, qdynamic)
#plt.title('Изменение количества популяций с течением времени')
plt.xlabel('время')
plt.ylabel('количество популяций')

plt.savefig(outpass2, dpi=300, bbox_inches='tight')